...
const { OTPResponse, OTPVerificationResponse } = require('./otpResponse');
const { OTPLESS_KEY_API, OTPLESS_USER_INFO_API, OTPLESS_MAGIC_LINK_API, OTPLESS_SEND_OTP_URL, OTPLESS_RESEND_OTP_URL, OTPLESS_VERIFY_OTP_URL } = require('./constants');
...
class UserDetail {
  ...
  static async sendOTP(phoneNumber, email, channel, hash, orderId, expiry, otpLength, clientId, clientSecret) {
    return sendOTP(phoneNumber, email, channel, hash, orderId, expiry, otpLength, clientId, clientSecret, null);
  }

  static async sendOTP(phoneNumber, email, channel, hash, orderId, expiry, otpLength, clientId, clientSecret, templateId) {
    try {
      const data = {};
      ...
      if (templateId) {
        ...
        const response = await axios.post(OTPLESS_SEND_OTP_URL, data, { headers });
        return response.data;
    } catch (error) {
        return handleOTPErrorResponse(error);
    }
  }


  static async resendOTP(orderId, clientId, clientSecret) {
    try {
        const data = {
            orderId: orderId
        };
        ...
        const response = await axios.post(OTPLESS_RESEND_OTP_URL, data, { headers });
        return response.data;
    } catch (error) {
        return handleOTPErrorResponse(error);
    }
  ...
  static async verifyOTP(email, phoneNumber, orderId, otp, clientId, clientSecret) {
    try {
        const data = {
            orderId: orderId,
            otp: otp
        ...
        const response = await axios.post(OTPLESS_VERIFY_OTP_URL, data, { headers });
        return response.data;
    } catch (error) {
        return handleOTPVerificationErrorResponse(error);
    }
  }
}

async function handleOTPErrorResponse(error) {
  if (error.response) {
      return OTPResponse.failedRes(error.response.data.message || 'Unknown error');
  } else {
      return OTPResponse.failedRes('An unexpected error occurred: ' + error.message);
  }
}

async function handleOTPVerificationErrorResponse(error) {
  if (error.response) {
      return OTPVerificationResponse.failedRes(error.response.data.message || 'Unknown error');
  } else {
      return OTPVerificationResponse.failedRes('An unexpected error occurred: ' + error.message);
  }
}
...
module.exports = {
  ...
  sendOTP: UserDetail.sendOTP,
  resendOTP: UserDetail.resendOTP,
  verifyOTP: UserDetail.verifyOTP
};
